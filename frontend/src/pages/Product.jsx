		...
		<div className='border-t -2 pt-5 transition-opacity ease-in duration-500 opacity-100'>
			...
			<div className='flex gap-12 sm:gap-12 flex-col sm:flex-row'>
				...
				<div className='flex-1'>
					...
					<p className='text-gray-500 mb-5 md:w-4/5'>{productData.description}</p>
					<div className='flex flex-col gap-4 my-8'>
						<p>Select size</p>
						<div className='flex gap-2'>
							{productData.sizes.map((item, index) => (
								<button onClick={() => setSize(item)} className={`border px-4 py-2 bg-gray-100 ${item === size ? 'border-orange-500' : ''}`} key={index}>
									{item}
								</button>
							))}
						</div>
					</div>
					<button onClick={() => addToCart(productData._id, size)} className='bg-black text-white px-3 text-sm active:bg-gray-700'>
						ADD TO CART
					</button>
			...
			<div className='mt-20'>
				...
				<div className='flex flex-col gap-4 border px-6 py-6 text-gray-5 00'>
